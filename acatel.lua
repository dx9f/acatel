-- if you gonna skid please give credits atleast.
-- if not get_user_info then error("there was error getting user info") return end
local info = {
    user = "Human" -- Lua Username
}
      -- requires
...
local filesystem = (function()
    ...
    tbl.get_files = function()
        ...
        remove_search_paths(call, "acatel_configs")
        add_search_path(call, "./csgo/acatel/", "acatel_configs")
        local file_names = {}
        local file = find_first_ex(call, "*", "acatel_configs", file_handle)
        while file ~= nil do
...
local backup = {}
local clantag = {
    tag = {"", "a", "ac", "aca", "acat", "acate", "acatel", "acatel", "acatel.", "acatel.u", "acatel.us", "acatel.us", "acatel.us", "acatel.s", "acatel.", "acatel", "acate", "acat", "aca", "ac", "a", "", ""}
}
...
local configs = {
    dir = "acatel/",
    default_dir = "acatel/default.cfg",
...
local ui = {
    nav = {
        ...
        text = groups.nav:text("Welcome to acatel: " .. info.user),
...
screen.watermark = function(self)
    local watermark = tools.animated_text(fonts.pixel, "A C A T E L", ui.accent:get(), vec2_t(screen.size.x / 2, screen.size.y - 15), 1)
    watermark()
end
...
screen.indicators = function(self)
    ...
    if ui.visuals.indicators:get() == 2 then
        ...
        local gradient = tools.animated_text(fonts.pixel, "ACATEL", ui.accent:get(), vec2_t(screen.size.x / 2 + binds.movetext + 1, screen.size.y / 2 + self.positions[binds.style][1]), 1)
        gradient()
    elseif ui.visuals.indicators:get() == 3 then
        ...
        local gradient = tools.animated_text(fonts.verdanabold, 'acatel', ui.accent:get(), vec2_t(screen.size.x / 2 + binds.movetext, screen.size.y / 2 + self.positions[binds.style][1]), 1)
        gradient()
    elseif ui.visuals.indicators:get() == 4 then
        ...
        local gradient = tools.animated_text(fonts.tahomabold, "Acatel*", ui.accent:get(), vec2_t(screen.size.x / 2 + 3, screen.size.y / 2 + self.positions[binds.style][1]), 0)
        gradient()
    end
end
